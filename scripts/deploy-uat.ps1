...
# This script deploys the SmartCenter applications to UAT environment
...
Write-Host "Starting UAT Environment Deployment..." -ForegroundColor Green
Write-Host "Build Path: $BuildPath" -ForegroundColor Yellow
Write-Host "Environment: $Environment" -ForegroundColor Yellow

try {
    # Create UAT directory if it doesn't exist
    ...
    Write-Host "UAT Environment Deployment completed successfully!" -ForegroundColor Green
    ...
    Write-Host "`nDo you want to deploy to Production? (Y/N)" -ForegroundColor Yellow
    $response = Read-Host
    if ($response -eq "Y" -or $response -eq "y") {
        Write-Host "Starting Production deployment..." -ForegroundColor Green
        & ".\scripts\deploy-production.ps1" -BuildPath $BuildPath
    }
